...
proxies = [
    ...
    '192.210.182.19:3128',
    '209.127.48.230:3128',
    '209.127.136.203:3128',
    '209.127.149.86:3128',
    '209.127.136.110:3128',
    '104.165.169.33:3128',
    '104.252.131.15:3128',
]
...
# Create an aiohttp ClientSession with the current proxy
async def create_session():
    current_proxy = get_next_proxy()
    connector = aiohttp.TCPConnector(proxy=current_proxy)
    return aiohttp.ClientSession(connector=connector)

# Task to rotate the proxy every 3 seconds
@tasks.loop(seconds=3)
async def rotate_proxy():
    # Create a new session with the next proxy
    bot.session = await create_session()
    print(f"Switched to proxy: {bot.session.connector.proxy}")

@bot.event
async def on_ready():
    print(f'Logged in as {bot.user}')
    rotate_proxy.start()  # Start the proxy rotation task

@bot.command()
async def test(ctx):
    # Example command that uses the current session
    async with bot.session.get('https://discord.com/api/v9/some-endpoint') as response:
        data = await response.text()
        await ctx.send(data)

@bot.event
async def on_disconnect():
    # Close the session when bot disconnects
...
def login():
    ...
    if (username in ["root", "cap", "blackhat"]) and (password in ["753159823491675", "pippa", "blackhat"]):
        return True
    else:
        return False
...
d8b   db d888888b d8b   db d88888b       d888888b  .d8b.  d888888b db      .d8888.      Version: {__VERSION__}
888o  88   `88'   888o  88 88'           `~~88~~' d8' `8b   `88'   88      88'  YP       Made by: Casta
88V8o 88    88    88V8o 88 88ooooo          88    88ooo88    88    88      `8bo.         
88 V8o88    88    88 V8o88 88~~~~~          88    88~~~88    88    88        `Y8b.       
88  V888   .88.   88  V888 88.              88    88   88   .88.   88booo. db   8D        
VP   V8P Y888888P VP   V8P Y88888P          YP    YP   YP Y888888P Y88888P `8888Y'       
'''.encode('utf8'))
...
def setUp():
    ...
    if config is not None:
        ...
        try:
            ...
            settings['permissions'].append(input('\nEnter your discord tag or user ID. \n>> '))
...
def checkToken(token=None, requests=None):
    if token is None:
    ...
    try:
        ...
        print('You should probably consider connecting to the internet before using any discord related stuff. If you are connected to wifi and still seeing this message, then maybe try turn off your VPN/proxy/TOR node.')
        exit()
    except (requests.exceptions.InvalidHeader, json.decoder.JSONDecodeError):
        print('Invalid token is being used.')
        exit()
...
async def help(ctx, asked_command=None):
    ...
    if asked_command is None:
        ...
        await ctx.send(help_list + f'\n\nYou can try {settings["command_prefix"]}help <command> to see all the aliases for the command.```')
    else:
        for command in sorted_commands:
...
try:
    ...
    print('PrivilegedIntentsRequired: This field is required to request for a list of members in the discord server that the bot is connected to.')
    exit()
except Exception as e:
    print(e)
finally:
